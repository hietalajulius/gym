    ...
    if sim.data.ctrl is not None:
        for i in range(action.shape[0]):
            if sim.model.actuator_biastype[i] == 0:
                ...
                # Increase prev command instead
                #sim.data.ctrl[i] = sim.data.ctrl[idx] + action[i]
...
def disable_weld(sim, body1, body2):
...
def enable_weld(sim, body1, body2):
    body1_id = sim.model.body_name2id(body1)
    body2_id = sim.model.body_name2id(body2)
